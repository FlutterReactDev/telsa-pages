...
// Hero Stats
window.teslahero = function () {
	var MAIN_DELAY = 730;

	var element = document.body.querySelector("#hero");

	var isMobile = false;

	//Init mobile hero height
	setTimeout(function () {
			element.classList.add("active");
	}, 0);

	var elementStats = element.querySelector(".stats");

	var speedometerNeedle = elementStats.querySelector(".speedometer-needle");
	var speedometerCircle = elementStats.querySelector(".speedometer-circle-inner");
	var speedNumbers = elementStats.querySelectorAll(".number-inner");
	var speedRotateValue1 = speedNumbers[0].getAttribute("data-timeincrement");
	var speedRotateValue2 = speedNumbers[1].getAttribute("data-timeincrement");
	var seatNumber = elementStats.querySelector(".number-inner");
	var arrowDown = element.querySelector(".arrow-down");
	var lis = elementStats.querySelectorAll(".stat");

	function getYAxisFromTimeIncrement(increment) {
			var yAxisCoordinate = {
					1: -33.5,
					2: -66.5,
					3: -100.5,
			...
			//TWS-26566: Check for IE11
			if (!!window.MSInputMethodContext && !!document.documentMode) {
					return yAxisCoordinate[increment] ? yAxisCoordinate[increment] - 152 : -489.5;
			}
			return yAxisCoordinate[increment] ? yAxisCoordinate[increment] : -337.5;
	}
	...
	TweenMax.to(speedNumbers[0], 1.733, {
			y: getYAxisFromTimeIncrement(speedRotateValue1),
			ease: Expo.easeInOut,
	});
	TweenMax.to(speedNumbers[1], 1.733, {
			y: getYAxisFromTimeIncrement(speedRotateValue2),
			ease: Expo.easeInOut,
	});
	...
	TweenMax.staggerTo(
			...
			{
					opacity: 1,
					delay: MAIN_DELAY / 1000,
			},
			0.133
	);

	TweenMax.to(arrowDown, 1, {
			opacity: 1,
			delay: 2,
	});

	var megamenuEl = document.getElementById("tds-menu-header-main");
	var headerLitemenuEl = document.getElementById("tds-site-header-wrapper");
	var showReserveNav = false;
	var reserveNavEl = document.body.querySelector(".reserve-header");
	var reserveNavTO;
	var tslaMenuHeader = document.getElementById("tsla-header-main");

	if (!document.body.contains(megamenuEl) && !document.body.contains(headerLitemenuEl)) {
			document.addEventListener(
					"scroll",
					function () {
							if (window.pageYOffset > window.innerHeight && !showReserveNav) {
									showReserveNav = true;
									reserveNavEl.classList.add("show");
									if (document.body.contains(tslaMenuHeader)) {
											tslaMenuHeader.classList.add("scrolled");
									}
									if (reserveNavTO) {
											clearTimeout(reserveNavTO);
									}
									reserveNavEl.style.opacity = "1";
							}

							if (window.pageYOffset <= window.innerHeight && showReserveNav) {
									showReserveNav = false;
									reserveNavEl.classList.remove("show");
									if (document.body.contains(tslaMenuHeader)) {
											tslaMenuHeader.classList.remove("scrolled");
									}
									reserveNavTO = setTimeout(function () {
											reserveNavTO = null;
											reserveNavEl.style.opacity = "0";
									}, 300);
							}
					},
					false
	...
	if (document.body.contains(headerLitemenuEl)) {
			reserveNavEl.style.zIndex = 0;
	}
	...
	var arrowEl = element.querySelector(".arrow-down");
	arrowEl.addEventListener(
			"click",
			function () {
...
window.teslavideo = function () {
	var videoEl = document.body.querySelector("#video video");
	var isMobile = false;
	var desktopSrc = videoEl.src;
	var ttp = document.body.querySelector("#video .ttp");

	if (window.innerWidth <= 550) {
			isMobile = true;
			videoEl.src = videoEl.dataset.mobile;
	}

	window.addEventListener("resize", function () {
			if (window.innerWidth <= 550 && !isMobile) {
					isMobile = true;
					videoEl.src = videoEl.dataset.mobile;
					videoEl.play();
			}
...
window.teslascroll = function () {
	//Init scroll reveal effect
	var scrollRevealEls = document.body.querySelectorAll("#wrapper .scroll-reveal");
	var videoEls = document.body.querySelectorAll("#wrapper video");
	var imageOverEl = document.body.querySelector("#topview .image-over");
	var isImageOver = false;
	var onScroll = function () {
			var scrollMult = window.innerWidth / 1280;
			if (scrollMult > 1) {
					scrollMult = 1;
			}
			for (var i = 0; i < scrollRevealEls.length; i++) {
					if (!scrollRevealEls[i].classList.contains("active")) {
							var offset = 0;
							if (scrollRevealEls[i].dataset && scrollRevealEls[i].dataset.offset) {
									offset = parseFloat(scrollRevealEls[i].dataset.offset) * scrollMult;
							}
							if (scrollRevealEls[i].getBoundingClientRect().top + offset < window.innerHeight) {
									scrollRevealEls[i].classList.add("active");
							}
					} else {
							if (scrollRevealEls[i].getBoundingClientRect().top >= window.innerHeight) {
									scrollRevealEls[i].classList.remove("active");
							}
					}
			}

			if (imageOverEl.getBoundingClientRect().top + imageOverEl.getBoundingClientRect().height / 2.1 < window.innerHeight / 2 && !isImageOver) {
					imageOverEl.classList.add("active");
					isImageOver = true;
			}

			if (imageOverEl.getBoundingClientRect().top + imageOverEl.getBoundingClientRect().height / 2.1 >= window.innerHeight / 2 && isImageOver) {
					imageOverEl.classList.remove("active");
					isImageOver = false;
			}

			for (var i = 0; i < videoEls.length; i++) {
					if (videoEls[i].getBoundingClientRect().top < window.innerHeight) {
							videoEls[i].play();
					}
			}
	};
